"""Integration tests for ShippingService - demonstrates end-to-end regression."""
...
class TestShippingService:
    ...
    @pytest.mark.xfail(reason="KNOWN BUG: Mixed products throw NoValidShippingMethodFoundException")
    def test_mixed_product_working_scenario(self):
        """
        REGRESSION TEST: Previously working scenario now fails.
        
        Scenario: Customer orders a battery-powered glass decorative lamp
        - Product is MIXED (both hazardous and fragile)
        - Expected: Should return GROUND and PRIORITY_GROUND options with increased volumetric cost
    ...
    @pytest.mark.xfail(reason="KNOWN BUG: get_recommended_shipping fails for mixed products")
    def test_get_recommended_for_mixed_product(self):
        """
        REGRESSION TEST: Getting recommended shipping for mixed product fails.
        
        Common use case where customer requests the cheapest available option.
        """
        product = Product(
            id="P105",
