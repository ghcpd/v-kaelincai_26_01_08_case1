...
class VolumetricCalculator:
    ...
    def calculate_volumetric_weight(order: Order) -> float:
        ...
        if order.has_fragile():
            # Mixed items (fragile + hazardous) need additional reinforced packaging
            # to safely encase hazardous components inside fragile packaging. Increase
            # the multiplier so volumetric cost can reflect the extra packaging.
            if order.has_hazardous():
                adjusted_volume = base_volume * 2.0
            else:
