...
class VolumetricCalculator:
    ...
    def calculate_volumetric_weight(order: Order) -> float:
        ...
        if order.has_fragile():
            multiplier = VolumetricCalculator.FRAGILE_VOLUME_MULTIPLIER

            # Mixed items (hazardous + fragile) may need extra packaging;
            # increase multiplier to ensure volumetric handling is captured
