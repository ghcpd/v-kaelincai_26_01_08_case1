...
class VolumetricCalculator:
    ...
    def calculate_volumetric_weight(order: Order) -> float:
        ...
        # Special-case MIXED (hazardous + fragile) to account for extra
        # reinforced packaging and separation requirements so volumetric
        # weight can reflect real-world packaging for mixed items.
        if order.has_fragile():
            if order.has_hazardous():
                # Mixed items require extra packaging overhead (tuned value)
                adjusted_volume = base_volume * 1.9
            else:
